...
func NewMySQLConnectionEnv() *MySQLConnectionEnv {
	return &MySQLConnectionEnv{
		Host:     getEnv("MYSQL_HOST", "10.165.27.103"),
		Host2:    getEnv("MYSQL_HOST2", "10.165.27.102"),
...
//ConnectDB isuumoデータベースに接続する
func (mc *MySQLConnectionEnv) ConnectDB2() (*sqlx.DB, error) {
	dsn := fmt.Sprintf("%v:%v@tcp(%v:%v)/%v", mc.User, mc.Password, mc.Host2, mc.Port, mc.DBName)
	return sqlx.Open("mysql", dsn)
}
...
func main() {
	...
	db2, err = mySQLConnectionData.ConnectDB2()
	if err != nil {
		e.Logger.Fatalf("DB connection failed : %v", err)
	}
	db2.SetMaxOpenConns(512)
	defer db2.Close()
...
func postChair(c echo.Context) error {
	...
	_, err = tx.Exec("INSERT INTO chair(id, name, description, thumbnail, price, height, width, depth, color, features, kind, popularity, stock) VALUES "+strings.Join(placeholders, ","), args...)
	if err != nil {
...
func searchChairs(c echo.Context) error {
	...
	if c.QueryParam("heightRangeId") != "" {
		...
		conditions = append(conditions, "heightrange = ?")
		params = append(params, RangeIndex)
	}
	...
	if c.QueryParam("depthRangeId") != "" {
		...
		conditions = append(conditions, "depthrange = ?")
		params = append(params, RangeIndex)
	}
	...
	limitOffset := " ORDER BY sort_key LIMIT ? OFFSET ?"
...
func getRangeIndex(cond RangeCondition, rangeID string) (int, error) {
	RangeIndex, err := strconv.Atoi(rangeID)
	if err != nil {
		return -1, err
	}
...
func postEstate(c echo.Context) error {
	...
	_, err = tx.Exec("INSERT INTO estate(id, name, description, thumbnail, address, latitude, longitude, rent, door_height, door_width, features, popularity) VALUES "+strings.Join(placeholders, ","), args...)
	if err != nil {
...
func searchEstates(c echo.Context) error {
	...
	if c.QueryParam("doorHeightRangeId") != "" {
		...
		conditions = append(conditions, "door_heightrange = ?")
		params = append(params, RangeIndex)
	}
...
func searchRecommendedEstateWithChair(c echo.Context) error {
	...
	h := chair.Height
	d := chair.Depth
	query = `SELECT id, name, description, thumbnail, address, latitude, longitude, rent, door_height, door_width, features, popularity FROM estate WHERE (door_width >= ? AND door_height >= ?) OR (door_width >= ? AND door_height >= ?) OR (door_width >= ? AND door_height >= ?) OR (door_width >= ? AND door_height >= ?) OR (door_width >= ? AND door_height >= ?) OR (door_width >= ? AND door_height >= ?) ORDER BY sort_key LIMIT ?`
...
func searchEstateNazotte(c echo.Context) error {
	...
	query := fmt.Sprintf(`SELECT id, name, description, thumbnail, address, latitude, longitude, rent, door_height, door_width, features, popularity FROM estate WHERE latitude <= ? AND latitude >= ? AND longitude <= ? AND longitude >= ? AND ST_Contains(ST_PolygonFromText(%s), geo) ORDER BY sort_key LIMIT 50`, coordinates.coordinatesToText())
